## **Final Project: Feature Selection & Model Evaluation**
...
TrafficDF$Date.time    <- as.POSIXlt(TrafficDF$Date.time, format = '%Y-%m-%d %H:%M:%S')  #convert to datetime object
...
#### Subset the Dataset to focus on 'Warnings' & 'Citations' Only
```{r}
...
#### EDA of 'Warnings' & 'Citations' Subset
_Note: Not all plots are not run; however, all plots are available on the team's GitHub Repo._
```{r}
# Preliminary Plots -----------------------------------------------------------
#bar chart of response variable
g <- ggplot(subDF, aes(Violation.Type)) +
      geom_bar()
...
subDF %>% 
  ...
  count(Violation.Type) %>% 
  mutate(DailyFreq = n/sum(n))
#^^day of month appears not significant factor in citation v. warning
...
#### Evaluate General Boosted Model for Prediction
```{r}
...
#load the model
load('gbm_model.rda')
rfDF$Hour <- as.numeric(rfDF$Hour)
...
#### Evaluate Random Forest for Prediction
_Note: This segment is not run due the time consuming nature of constructing multiple models of this size. Detailed results are available on the team's GitHub Repo and report._
```{r, eval = FALSE}
#construct a train/test dataframe
trainDF <- rfDF[sample(nrow(rfDF), size = nrow(rfDF)*.70), ]
...
#### Run 'RandomForestExplainer' to Better Understand Prediction Elements
```{r, warning = FALSE}
...
##generate pretty plot for report and presentation
...
coefDF[order(coefDF[,1], decreasing = TRUE),]
...
##rebuild and rerun the model by removing poor coefficients and those potential collinearity conflicts
regDF$Color       <- NULL
regDF$DLState     <- NULL
